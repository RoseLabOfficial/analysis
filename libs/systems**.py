...
class WholeCellStimulus:
    ...
    def estimate_ge_gi_piecewise_linear(
        ...
        """
        Estimate ge(t), gi(t) under a piecewise-linear-in-time model on bin-edge nodes.

        Discretization choices (mathematically consistent):
        - Each bin k uses a HALF-OPEN sample interval [l_idx[k], r_idx[k]) so samples are not double-counted.
        - Integrals are approximated using MIDPOINT quadrature on that bin's sample grid.
        - Conductances are linear on each bin: g(t) = (1-s) g_k + s g_{k+1}, s in (0,1).

        This yields a sparse linear least-squares system with optional positivity constraints.
        """
        ...
        if nonlinear:
            Q = np.stack(self.binned_timeseries["target Qsyn nonlinear"].to_numpy()).astype(np.float64).T
        else:
            Q = np.stack(self.binned_timeseries["target Qsyn"].to_numpy()).astype(np.float64).T
        # Q: (Nclamps, Nbins)

        Nbins = len(l_idx)
        Nnodes = Nbins + 1
        Nclamps = self.Nclamps
        ...
        # Build sparse system A x ≈ y
        # x = [ge_nodes(0..Nnodes-1), gi_nodes(0..Nnodes-1)]
        n_rows = Nbins * Nclamps
        n_cols = 2 * Nnodes
        ...
        for k, (l, r) in enumerate(zip(l_idx, r_idx)):
            ...
            # ---- HALF-OPEN bin: [l, r) ----
            # This is the key mathematical fix: no endpoint duplication across bins.
            seg = Vm[:, l:r]  # (Nclamps, n)
            n = seg.shape[1]

            # Fill RHS for this bin (all clamps)
            y[row0:row0 + Nclamps] = Q[:, k]

            if n == 0:
                # Degenerate bin (shouldn't happen, but keep system consistent)
                row0 += Nclamps
                continue

            # ---- MIDPOINT quadrature points in (0,1) ----
            # s_j = (j + 0.5)/n, j=0..n-1
            s = (np.arange(n, dtype=np.float64) + 0.5) / float(n)
            w0 = 1.0 - s
            w1 = s

            # Weighted integrals:
            # Phi_e0[c] = ∫ (1-s)(Ee - Vm_c(t)) dt  ≈ dt * Σ (Ee - Vm_c[j]) * w0[j]
            # Phi_e1[c] = ∫ s(Ee - Vm_c(t)) dt      ≈ dt * Σ (Ee - Vm_c[j]) * w1[j]
            Ee_minus_V = Ee - seg
            Ei_minus_V = Ei - seg

            Phi_e0 = dt * np.sum(Ee_minus_V * w0[None, :], axis=1)  # (Nclamps,)
            Phi_e1 = dt * np.sum(Ee_minus_V * w1[None, :], axis=1)
            Phi_i0 = dt * np.sum(Ei_minus_V * w0[None, :], axis=1)
            Phi_i1 = dt * np.sum(Ei_minus_V * w1[None, :], axis=1)
        ...
        # Bounds (positivity on BOTH ge and gi nodes)
        if constrained:
            lb = np.zeros(n_cols, dtype=np.float64)
            ub = np.full(n_cols, np.inf, dtype=np.float64)
        else:
            lb = np.full(n_cols, -np.inf, dtype=np.float64)
            ub = np.full(n_cols,  np.inf, dtype=np.float64)
        ...
        # Store nodes separately (Nbins+1)
        self.node_timeseries[f"ge nodes{suffix}"] = ge_nodes
        self.node_timeseries[f"gi nodes{suffix}"] = gi_nodes

        # Bin-midpoint series for comparison to stepwise (Nbins)
        self.binned_timeseries[f"ge{suffix}"] = 0.5 * (ge_nodes[:-1] + ge_nodes[1:])
        self.binned_timeseries[f"gi{suffix}"] = 0.5 * (gi_nodes[:-1] + gi_nodes[1:])

        # Optional diagnostics
        resid = (A @ x) - y
        resid = resid.reshape(Nbins, Nclamps).T  # (Nclamps, Nbins)
        self.binned_timeseries[f"ge/gi residual rms{suffix}"] = np.sqrt(np.mean(resid**2, axis=0))
